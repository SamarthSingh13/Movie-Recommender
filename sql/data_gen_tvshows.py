...
filename = "../kaggle_datasets/tv_shows.csv"
fields = []
rows = []
...
with open(filename, 'r',encoding='utf-8') as csvfile:
    ...
    for row in csvreader:
        ...
        for i in range(leng):
            if row[i]=="":
                row[i]=None
        query_insert = "INSERT INTO show(id,title,release_year,imdb_rating,rotten_tomato,is_movie) VALUES (%s, %s, %s, %s, %s, %s);"
        crsr.execute(query_insert,(row[0],row[1],row[2],row[4],row[5],0))
